...
import { Battery, Wifi, Cpu, Clock, CheckCircle, AlertCircle } from "lucide-react";
...
export function HealthWidget({ device }) {
  ...
  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 sm:p-5 flex flex-col justify-between h-full">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[11px] font-mono text-emerald-400">{device.device_id_code}</span>
          </div>
        </div>

        <div className="flex items-center gap-1.5">
          <span className={`w-2 h-2 rounded-full ${isOnline ? "bg-emerald-400 animate-ping" : "bg-rose-500"}`} />
          <span className={`text-xs font-semibold uppercase ${isOnline ? "text-emerald-400" : "text-rose-400"}`}>
            {device.status}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 my-2">
        {/* Battery */}
        <div className={`p-2.5 rounded-xl border ${isLowBattery ? "bg-rose-950/40 border-rose-500/50 text-rose-300" : "bg-slate-950/60 border-slate-800 text-slate-200"}`}>
          ...
          <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
            <div
              className={`h-full rounded-full ${isLowBattery ? "bg-rose-500 animate-pulse" : "bg-emerald-400"}`}
              style={{ width: `${battery}%` }}
            />
          </div>
        </div>
        ...
        <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-200">
          ...
          <div className="text-[10px] text-emerald-400 font-mono">Signal: Excellent</div>
        </div>
      </div>
