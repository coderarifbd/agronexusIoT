      ...
      <div className="w-full max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl flex flex-col h-[88vh] max-h-[640px] overflow-hidden transition-colors">
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === "chat" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                }`}
              >
                Chat
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === "anomalies" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                }`}
              >
                Anomalies
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === "forecast" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                }`}
              >
                Forecast
              </button>
        ...
        {activeTab === "chat" && (
          <div className="flex-1 flex flex-col justify-between overflow-hidden">
            ...
            <div className="px-4 py-2 bg-slate-50 dark:bg-slate-950/60 border-t border-slate-200 dark:border-slate-800 flex items-center gap-2 overflow-x-auto text-[11px]">
              <span className="text-slate-400 dark:text-slate-500 font-semibold shrink-0">Try:</span>
              <button
            ...
            >
              <input
                ...
                className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none"
              />
              <button
                type="submit"
                disabled={loading}
                className="p-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl transition-colors disabled:opacity-50 shadow-sm"
              >
        ...
        {activeTab === "anomalies" && (
          <div className="flex-1 p-5 overflow-y-auto space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={loadAnomalies} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
                Refresh Scanner
              </button>
            </div>
