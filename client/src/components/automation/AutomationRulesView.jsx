        ...
        <div className="space-y-3">
          ...
          {rules.map((rule) => {
            ...
            return (
              <div
                key={rule.id}
                className="bg-slate-950/80 border border-slate-800/90 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4"
              >
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${
                        rule.is_active ? "bg-emerald-400 animate-ping" : "bg-slate-600"
                      }`}
                    />
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                    <span className="bg-emerald-950/60 text-emerald-300 px-2.5 py-1 rounded-lg border border-emerald-500/30">
                      IF{" "}
                      {conditions.map((c, i) => (
                        <span key={i}>
                          <strong className="text-white">{c.field_key}</strong> {c.operator} {c.value}
                        </span>
                      ))}
                    </span>
                    <span className="text-slate-500 font-bold">➔</span>
                    <span className="bg-blue-950/60 text-blue-300 px-2.5 py-1 rounded-lg border border-blue-500/30">
                      THEN{" "}
                      {actions.map((a, i) => (
                        <span key={i}>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-4">
        ...
        <div className="space-y-3">
          ...
          {scheduled.map((item) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-purple-950/50 border border-purple-500/30 flex items-center justify-center text-purple-400 font-mono font-bold text-xs">
                  {item.time_schedule}
                </div>
                <div>
                  <h4 className="text-sm font-bold text-white">{item.name}</h4>
                  <div className="text-xs font-mono text-slate-400 mt-0.5">
                    Target: <span className="text-emerald-400">{item.actuator_name || "Actuator Relay"}</span> ➔ Set to{" "}
      ...
      {showRuleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-4">Create IF-THEN Automation Rule</h3>
            <form onSubmit={handleCreateRule} className="space-y-4">
              ...
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <input
                    ...
                    className="bg-slate-900 border border-slate-800 rounded-xl px-2 py-2 text-xs text-white font-mono"
                    required
                  />
                </div>
              </div>
              ...
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                ...
                <div>
                  <label className="block text-[11px] text-slate-400 mb-1">Send In-App Alert (Optional)</label>
                  <input
                    type="text"
                    value={alertMessage}
      ...
      {showScheduleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-4">Create Scheduled Time Routine</h3>
            <form onSubmit={handleCreateSchedule} className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Trigger Time (HH:MM)</label>
                <input
                  type="time"
                  value={schedTime}
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Target Actuator Relay</label>
                <select
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Action State</label>
                <select
                  value={schedState}
