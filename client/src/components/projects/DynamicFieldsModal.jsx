...
import { X, Plus, Trash2, Sliders, Thermometer, Droplets, Gauge, Wind, Sun, Activity, Zap } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs font-mono text-slate-400">
                      Key: <span className="text-emerald-400">{f.field_key}</span> • Unit: {f.unit || "None"} • Range: [{f.min_value} – {f.max_value}]
                    </div>
                  </div>
                </div>
        ...
        {/* Add New Field Form */}
        <form onSubmit={handleAddField} className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-slate-400 mb-1">Field Label / Name</label>
              <input
                type="text"
                value={name}
                onChange={(e) => {
                ...
                className="w-full bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-xl px-3 py-2 text-sm text-white focus:outline-none"
                required
              />
            </div>

            <div>
              <label className="block text-xs text-slate-400 mb-1">JSON Key (Device Payload Key)</label>
              <input
                type="text"
                value={fieldKey}
                onChange={(e) => setFieldKey(e.target.value)}
                placeholder="e.g. soil_salinity"
                className="w-full bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-xl px-3 py-2 text-sm text-emerald-400 font-mono focus:outline-none"
                required
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-xl px-3 py-2 text-sm text-white focus:outline-none font-mono"
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-emerald-950/30 flex items-center gap-2"
            >
              <Plus className="w-4 h-4" />
